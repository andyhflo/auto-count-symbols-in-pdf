...
function trace(c, div) {
    ...
    function displaySVG(size) {
        displayContents(getSVG(size), div);
    }

}
function openPDF(c) {

    var loadingTask = pdfjsLib.getDocument(c);
        ...
        for (let pageNumber = 1; pageNumber <= totalPages; pageNumber++) {
            pdf.getPage(pageNumber).then(function (page) {
                ...
                renderTask.promise.then(function () {
                    trace(canvas.toDataURL('image/png'), svgDiv)
                });
            });
        }
...
function displayContents(c, div) {
    ...
    for (var i = 0; i < p.length; i++) {
        ...
        path = [Math.round(Math.pow(Math.pow((maxX - minX), 2) + Math.pow((maxY - minY), 2), .5) * 10000) / 10000, newPath, maxX - minX, maxY - minY, midX, midY, i]
        paths.push(path);
    }
    ...
    var refs = '<use href="#0" x="' + paths[0][4] + '" y="' + paths[0][5] + '" fill="none" stroke="black"/>'
    var repeats = 0;
    var quantity = 1;
    ...
    for (var l = 1; l < paths.length; l++) {
        ...
        if (differences > threshold) {
            ...
            table.insertAdjacentHTML('beforeend', '<div class="row"><div class="col"><input type="text" maxlength="15" size="15" name="description" pattern=[A-Za-z][A-Za-z\d]{4,29} title="Description" placeholder="Description"></div><div class="col"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="-10 -10 20 20"  version="1.1"><path d="' + lookBack.toString().replace(/[,]/g, " ") + '" fill="none" stroke="black"/></svg></div><div class="col">' + quantity + '</div></div>');
            lookBack = next
            quantity = 1;
        } else {
            repeats++
        ...
        refs += '<use href="#' + (l - repeats) + '" x="' + paths[l][4] + '" y="' + paths[l][5] + '" fill="none" stroke="black"/>'
    }
    table.insertAdjacentHTML('beforeend', '<div class="row"><div class="col"><input type="text" maxlength="15" size="15" name="description" pattern=[A-Za-z][A-Za-z\d]{4,29} title="Description" placeholder="Description"></div><div class="col"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="-10 -10 20 20"  version="1.1"><path d="' + lookBack.toString().replace(/[,]/g, " ") + '" fill="none" stroke="black"/></svg></div><div class="col">' + quantity + '</div></div>');
    newSVG += '</g>'
    refs += '</svg>'
    div.insertAdjacentHTML('beforeend', newSVG + refs);
